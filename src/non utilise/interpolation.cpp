...
vec3 interpolation(float t, buffer<vec3> const& key_positions, buffer<float> const& key_times)
{
    ...
    vec3 p = cardinal_spline_interpolation(t,key_times[idx-1],key_times[idx  ],key_times[idx+1 ],key_times[idx+2], key_positions[idx-1],key_positions[idx  ],key_positions[idx+1],key_positions[idx+2],0.5f);
